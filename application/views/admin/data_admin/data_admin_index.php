...
<div class="content-wrapper">
   ...
   <section class="content">
      <!-- Default box -->
      <a href="<?= base_url('admin/data-admin/tambah') ?>"><button class="btn btn-success mb-2"><i class="fa fa-plus"></i> Tambah Data</button></a>
                ...
                <table id="data-tables" class="table table-bordered table-striped">
                   ...
                   <tbody>
                    <?php $no=1; foreach ($semua_admin as $admin): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td width="1%"><img class="img-circle elevation-2" src="<?= base_url('assets/img/admin/') ?><?= $admin->admin_gambar ?>" width="50"></td>
                            <td><?= $admin->admin_kode ?></td>
                            <td><?= $admin->admin_nama ?></td>
                            <td><?= $admin->admin_tanggal_dibuat ?></td>
                            <td width="15%">
                                <a href="<?= base_url('admin/').$controller.'/edit/'.$admin->adminid ?>"><button class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> Edit</button></a>
                                <a href="<?= base_url('admin/').$controller.'/hapus/'.$admin->adminid ?>" onclick="return confirm('Yakin Ingin Hapus data ?');"><button class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Hapus</button></a>
                            </td>
                        </tr>
